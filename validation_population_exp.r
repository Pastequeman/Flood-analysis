...
# Purpose:
# every year report the population exposure to flood for every country
...
mask <- mask %>% filter(is.na(country) != TRUE, is.na(position) != TRUE)
...
t <- as.vector(unique(mask$country))
complete_serie_pop_position <- tibble(years     = rep(seq(YEAR_STA, YEAR_END, 1), each = 4*100*length(t)),
                                      ite       = rep(rep(seq(1, 100), each = 4), times = length(seq(YEAR_STA, YEAR_END, 1))*length(t)),
                                      position  = rep(seq(1, 4), times = 100 * length(seq(YEAR_STA, YEAR_END, 1))*length(t)),
                                      country   = rep(rep(t, each = 4*100),
                                                      times = length(seq(YEAR_STA, YEAR_END, 1))),
                                      sum_max   = 0,
                                      sum_qua   = 0,
                                      sum_cmax  = 0,
                                      sum_cqua  = 0)
...
for (years in seq(YEAR_STA, YEAR_END, 1)) {
  ...
  for (i in 1:100) {
    #print(i) ## DEBUG
    potential_flood_max[i, ] <- potential_flood_max[i,] * flood_frac$pop * day7
    potential_flood_qua[i, ] <- potential_flood_qua[i,] * flood_frac$pop * day7
  }
  ...
  # save yearly files; if folder does not exist, create it
